...
- **Extension (ipphone):** tenant schema. Check ipphone columns vs Extension model and ExtensionRequest / updateableColumns.
- **Queue, Agent, Route, Ivr, InboundRoute, CustomApp, Tenant, HelpCore, Device, etc.:** same process — schema file → list columns → compare model + controller + any Form Request → remove or adjust references to dropped columns.
- **Instance tables:** device, tt_help_core, globals — same idea.
...
- [ ] Trunk: validation and controller aligned with `trunks` (no carrier/sipiaxpeer/sipiaxuser required on update).
- [ ] All other models/controllers/Form Requests: aligned with their tables.
...
| Task | Scope | Outcome |
|------|--------|----------|
| **1. Models vs DB** | All API models and their controllers/Form Requests vs instance + tenant SQL | No validation or model attributes for columns that don’t exist; Trunk (and others) fixed. |
